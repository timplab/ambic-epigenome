...
```{r limma_compare, eval = T, include = FALSE, cache=F }
# rowdata 
...
# start limma
design <- model.matrix( ~ 0 +  condition + day, data = colData)
...
for ( condition in unique(colData$condition)){
  ...
  fit <- lmFit(dat.sub ,design)
  fit <- contrasts.fit(fit,contrast.mat)
  fit <- eBayes(fit)
  # fit$coefficients are logFC
  # fit$p.value are p.values
  logfc <- bind_cols(rowData,as_tibble(fit$coefficients)) %>%
    gather(contrast,value,-chrom,-start,-end)
  pvals <- bind_cols(rowData,as_tibble(fit$p.value)) %>%
    gather(contrast,value,-chrom,-start,-end)
  # combine logfc and pval and BH multiple testing correction
  comp <- bind_rows(list(logfc = logfc, pval = pvals),.id = "what") %>%
    spread(what,value) %>%
    group_by(contrast) %>%
    mutate(adjusted.pval = p.adjust(pval,"BH"))
  comp.list[[condition]] <- comp
}
comp.tb <- bind_rows(comp.list,.id = "condition")

comp.tb %>%
  filter(adjusted.pval < 0.5) %>%
