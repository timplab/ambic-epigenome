...
plotdir <- "/home/isac/Dropbox (Timp Lab)/Data/ambic/aging_study/plots"
# data
...
plotdir <- "/home/isac/Dropbox/Data/ambic/aging_study/cnv"
...
```
# Let's try cn.MOPs
# cn.MOPs uses mixture of poisson distributions to estimate the copy numbers and compare them 
# https://academic.oup.com/nar/article/40/9/e69/1136601
...
# rowdata 
rowData <- cov.norm[,1:3]
# data matrix
dat.mat <- cov.norm[,-(1:3)]
# coldata; make sure column order is the same as pd order
colData <- pd.tb[match(colnames(dat.mat),pd.tb$id),] 
if (! sum(colData$id == colnames(dat.mat)) == ncol(dat.mat)) { message("make sure colData order matches data matrix order")}

#contrast.mat <- makeContrasts(
#  contrasts = c("day30-day0","day60-day0","day90-day0"),
#  levels = c("day0","day30","day60","day90"))
#cont.pvals <- 
#for ( condition in unique(colData$condition)){
#  print(condition)
#  dat.sub <- dat.mat[,colData$condition == condition]
#  colData.sub <- colData[colData$condition == condition,]
#  design <- model.matrix( ~ 0 +  day, data = colData.sub)
#  for (rowi in seq(nrow(dat.sub))){
#    datrow <- as.numeric(dat.sub[rowi,])
#    pvals <- sapply(seq(ncol(contrast.mat)),function(coni){
#      onei <- which(contrast.mat[,coni]==-1)
#      twoi <- which(contrast.mat[,coni]==1)
#      one <- datrow[which(design[,onei] == 1)]
#      two <- datrow[which(design[,twoi] == 1)]
#      t.test(one,two)$p.val
#    })
#    
#  }
#}
...
norm.avg <- norm.gather %>%
  na.omit() %>%
  group_by(cell,condition,day,chrom,start,end) %>%
  summarize(n = n(), totcov = sum(cov), cov = mean(cov))

# compared to day 0
cov.del <- norm.avg %>%
  ungroup() %>%
  dplyr::select(-n,-totcov,-cell) %>%
  spread(day,cov) %>%
  gather(day , cov,-chrom,-start,-end,-condition,-`0`) %>%
  mutate(
    del = cov - `0`,
    ratio = log2(cov/`0`))
# bin data
...
g <- ggplot(cov.del,aes(x = condition,fill = day, y = ratio)) +
...
cov.spread <- cov.del  %>%
  ungroup() %>%
  mutate(sample = paste0(condition,day)) %>%
...
pd.tb
days <- c("30","60","90")
labels.tb <- tibble(id= unique(segs.tb$ID)) %>%
  rowwise() %>%
  mutate(day = days[sapply(days,grepl,id)],
         condition = sapply(strsplit(id,day ),"[[",1),
         cell = cells[sapply(cells,grepl,condition)],
         media = medias[sapply(medias,grepl,condition)]
         ) %>%
  ungroup()
