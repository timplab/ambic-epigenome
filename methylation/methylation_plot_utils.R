...
tabix <- function(querypath,dbpath,col_names=NULL,verbose=TRUE){
        if ("GRanges" %in% class(dbpath)){
            ...
            if (verbose) cat(paste0("reading regions defined by GRanges object",
                                    " in ",querypath,"\n"))
            if (length(dbpath) > 4000 ){
                if (verbose) cat(paste0("regions cannot exceed 4000\n"))
                return()
            }
...
# bsseq
mfreqToBsseq <- function(data,label="samp",fill=0){
    ...
    meth.mat = as.matrix(dat.meth[,4:dim(dat.meth)[2]])
    cov.mat = as.matrix(dat.cov[,4:dim(dat.cov)[2]])
    # replace NA
