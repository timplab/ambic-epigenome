...
def parseArgs():
    ...
    parser.add_argument('-i,','--indir',dest='root',metavar='dir',required=True,
            type=os.path.abspath,help='common directory for fast5 files')
    parser.add_argument('-o','--outdir',metavar='dir',type=os.path.abspath,
            required=True,help='output directory')
    parser.add_argument('-n',metavar='N',type=int,default=4000,
            help='number of fast5s to group into one dir')
    parser.add_argument('-s','--sample',metavar='foo',type=str,required=False,
            help='name of the run to combine')
    parser.add_argument('-a','--archive',action='store_true',
            help='flag for archiving')
...
def combineRuns(args):
    ...
    totfiles = 0
    if args.dryrun :
        print("dryrun",flush=True)
        def updatecommand(path,newpath) :
            print("{} to {}".format(path,newpath),flush=True)
    else :
        os.mkdir(wdir)
        def updatecommand(path,newpath) :
